...
void handleClient(WiFiClient& client) {
    ...
    if (request.indexOf("GET / ") >= 0 || request.indexOf("GET /?") >= 0) {
        ...
        for (int i=0;i<6;i++){
            ...
            // Radio then label (number + name), all left-aligned
            li += "<label style='display:flex;align-items:center;gap:6px;margin:0'><input type='radio' name='currentPatternId' value='" + String(id) + "'" + (config.currentPatternId==id?" checked":"") + "> <span>" + labelForIdDash(id) + "</span></label>";
